...
export default function GuruApprovalPage() {
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('/api/guru/approval');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="page-header flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <span className="badge badge-amber mb-2">Approval Kelas</span>
          <h1>Approval Izin Siswa</h1>
          <p>Review izin dan sakit siswa di kelas Anda.</p>
        </div>
      ...
      {notification && (
        <div className="p-4 bg-[rgba(34,197,94,0.1)] border border-[rgba(34,197,94,0.2)] rounded-[var(--radius-input)] text-xs font-semibold text-[#4ade80] animate-slide-down">{notification}</div>
      )}
            ...
            <div className="p-5 space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-[var(--text-muted)] text-xs">{req.kelas.replace(/-/g, ' ')} - NIS: {req.nis}</p>
                </div>
                <span className={`badge ${req.tipe === 'IZIN' ? 'badge-amber' : 'badge-sky'}`}>{req.tipe}</span>
              </div>
              {req.tanggal && (
                <div>
                  <span className="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-wider block">Tanggal</span>
